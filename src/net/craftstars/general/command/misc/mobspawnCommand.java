...
public class mobspawnCommand extends CommandBase {
        ...
        case 2:
            try {
                ...
                if(n > 5 && Toolbox.lacksPermission(plugin, sender, "spawn mobs en masse", "general.mobspawn.mass"))
                    return true;
                if(args[0].equalsIgnoreCase("SpiderJockey"))
                    while(n-- > 0) doCompoundSpawn(sender, "Skeleton", "Spider", where);
                else while(n-- > 0) doSimpleSpawn(sender, args[0], where);
        ...
        case 3:
            try {
                ...
                if(args[0].equals("-") || args[1].equals("-"))
                    while(n-- > 0) doSimpleSpawn(sender, (args[0]+args[1]).replace("-", ""), where);
                else while(n-- > 0) doCompoundSpawn(sender, args[0], args[1], where);
            } catch(NumberFormatException e) {
                return false;
            }
            return true;
    ...
    private LivingEntity doSimpleSpawn(Player sender, String mobName, Location where) {
        ...
        if(split.length == 2) {
            if(!mob.setData(entity, sender, split[1]))
                Messaging.send(sender, "&cError setting the data.");
        }
        return entity;
    }
