...
public class whoCommand extends CommandBase {
	...
	public boolean fromPlayer(General plugin, Player sender, Command command, String commandLabel, String[] args) {
		if(args.length < 1 || commandLabel.equalsIgnoreCase("whoami")) {
			...
			if(args[0].equals("help")) return SHOW_USAGE;
			if(Toolbox.lacksPermission(plugin, sender, "view info on users other than yourself", "general.who",
	...
	private void showPlayerInfo(CommandSender sender) {
		...
		if(General.plugin.config.getBoolean("playerlist.show-ip", false)) {
			...
			} else if(sender instanceof ConsoleCommandSender) canSeeIp = true;
			if(canSeeIp) Messaging.send(sender, "&6 -&e IP: &f" + this.ip);
		}
		if(this.isAway)
			Messaging.send(sender, "&6 -&e Status: &f" + "Away (" + this.awayReason + ").");
		else Messaging.send(sender, "&6 -&e Status: &fAround.");
		Messaging.send(sender, "&f------------------------------------------------");
	}
