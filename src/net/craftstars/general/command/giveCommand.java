    ...
    public boolean fromPlayer(General plugin, Player sender, Command command, String commandLabel,
        ...
        if(args.length < 1) {
            Messaging.send(sender,
                    "&cUsage: /give [item(:type)|player] [item(:type)|amount] (amount)");
        ...
        if(dataType != -1) {
            if(!Items.validateType(itemId, dataType)) {
                Messaging.send("&f" + dataType + "&c is not a valid data type for &f"
                        + Items.name(itemId, -1) + "&c.");
                return true;
            }
        }
        ...
        if(dataType != -1) {
            if(slot < 0) {
                who.getWorld().dropItem(who.getLocation(),
                        new ItemStack(itemId, amount, ((byte) dataType)));
            } else {
