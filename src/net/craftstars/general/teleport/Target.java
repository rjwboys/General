...
public class Target {
    ...
    private Target(List<LivingEntity> l, TargetType t) {
        ...
        break;
        case OTHER:
            if(l.size() > 1) title = "several people";
            else {
                LivingEntity entity = l.get(0);
                if(entity instanceof Player) title = ((Player) entity).getDisplayName();
                else title = "someone";
            }
        break;
        case MOB:
            if(l.size() > 1) title = "several mobs";
            else {
                LivingEntity entity = l.get(0);
    ...
    public boolean hasPermission(Player who) {
        ...
        if(teleportees.size() > 1) {
            perm = perm && !Toolbox.lacksPermission(General.plugin, who, "teleport en masse", "general.teleport.mass");
        }
        return perm;
    }
    ...
    public static Target get(String targ, Player teleporter) {
        ...
        if(teleporter == null) notify = new ConsoleCommandSender(mc);
        ...
        if(teleporter != null) {
            ...
            if(Toolbox.equalsOne(targ, "there", "$there")) {
                ...
                if(victim != null) {
                    TargetType tt;
                    if(victim instanceof Player) tt = TargetType.OTHER;
